...
function App() {
  ...
  const getUsers = async () => {
    try {
      const res = await axios.get("https://back-prova.onrender.com/api/person");
      setList(res.data);
  ...
  const registerUser = async () => {
    try { 
      await axios.post("https://back-prova.onrender.com/api/person",{
  ...
  const deleteUser = async (id) => {
    try { 
      await axios.delete(`https://back-prova.onrender.com/api/person/${id}`);
      getUsers()
    } catch (error) {
      console.error("Erro ao inserir", error);
    }
      ...
      <div className="flex justify-center w-full">
        <form className="max-w-md mx-auto " onSubmit={handleSubmit}>
          <div className="relative z-0 w-full mb-5 group">
            <input onChange={(e) => {setName(e.target.value)}} type="text" name="floating_email" id="floating_email" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="Name " required />
          </div>
          <div className="relative z-0 w-full mb-5 group">
          ...
          <div className="relative z-0 w-full mb-5 group">
            <input onChange={(e) => {setSalary(e.target.value)}} type="text" name="floating_password" id="floating_password" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="Salary " required />
          </div>
